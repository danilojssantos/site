...
class StsAltOrdemTipoPgSite {
    ...
    private function verfTipoPgInferior() {
        ...
        $verTipoPg = new \App\adms\Models\helper\AdmsRead();
        $verTipoPg->fullRead("SELECT id, ordem FROM sts_tps_pgs WHERE ordem =:ordem", "ordem={$ordem_super}");
    ...
    private function exeAltOrdemTipoPg() {
        ...
        $upMvBaixo->exeUpdate("sts_tps_pgs", $this->Dados, "WHERE id =:id", "id={$this->DadosTipoPgInferior[0]['id']}");

        $this->Dados['ordem'] = $this->DadosTipoPg[0]['ordem'] - 1;
        $upMvCima = new \App\adms\Models\helper\AdmsUpdate();
        $upMvCima->exeUpdate("sts_tps_pgs", $this->Dados, "WHERE id =:id", "id={$this->DadosTipoPg[0]['id']}");
